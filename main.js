// метод JSON.stringify - объект переводит в строку; parse делает наоборот
fetch(
    'https://mdn.github.io/learning-area/javascript/oojs/json/superheroes.json'
)
    .then(res => res.json()) // Метод возвращает JS объект
...
function setMembers(members) {
    return members
        .map(hero => (
            `<div>
            <h3>${hero.name}</h3>
            <p>Secrett identity: ${hero.secretIdentity}</p>
            <p>Age: ${hero.age}</p>
            <p> Superpowers:</p>
            <ul>
